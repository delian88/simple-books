...
switch ($action) {
    ...
    case 'deleteCustomer':
        $data = json_decode(file_get_contents('php://input'), true) ?? [];
        $pdo->prepare("UPDATE customers SET is_archived = 1 WHERE id = ? AND company_id = ?")
    ...
    case 'deleteInvoice':
        $data = json_decode(file_get_contents('php://input'), true) ?? [];
        $pdo->prepare("DELETE FROM sales_invoice_lines WHERE invoice_id = ?")
            ->execute([$data['id']]);
        $pdo->prepare("DELETE FROM sales_invoices WHERE id = ? AND company_id = ?")
            ->execute([$data['id'], $companyId]);
        jsonResponse(['ok' => true]);
        break;
